...
// every game/company/genre/award page instead of one empty shell for the whole site. The
// captured HTML still contains the original <script type="module"> tag, so a real visitor's
// browser re-renders via React exactly as before — this only changes what a crawler (or a raw
// `curl`) sees on first response, not the live app's behavior.
...
// the build fail outright, so allow overriding it (PRERENDER_PORT=4323 npm run build).
