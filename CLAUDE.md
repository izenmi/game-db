...
**1191本(2026-08-12)**: シリーズ機能の実装中に**同一ゲームの二重登録**を1件見つけ、ユーザー指示で削除した(`paper-trail-2`。`paper-trail`と同じ『ペーパートレイル』/Newfangled Games/2024年5月/同一の表紙画像で、`developerIds`に同じ会社が2回入っていた)。**`apply_batch.py`のid衝突検出は同じidにしか効かない**ので、**日本語タイトルで登録済みの作品を英語名の別idで再投入するとすり抜ける**(`suggest-candidates.mjs`が英語名でしか照合できないことの帰結。CLAUDE.md「候補一覧を目視して既存の日本語タイトルを思い出す作業は省けない」と同じ穴)。同種の重複を洗うなら**タイトル正規化+開発元+発売年での突合**が要る(未実装)。
...
## 好みからおすすめ `/recommend`(2026-08-12。ranobe-dbからの移植)

好きなジャンルを最大3つ選ぶと、ジャンルの重なりが近いゲームを一致度つきで20本出す1ページ
(`src/ui/recommend/RecommendPage.tsx`)。

- **スコアは「このゲームが好きなら」と同じ式**をクライアント側で再現している(`idf(t)=log(N/df(t))` の
  IDF重み付きコサイン類似度)。**別の式を新造しないこと** — 詳細ページの並びと食い違うため。
  制作者ボーナスは種になるゲームがないので足さず、そのぶん上限が1.0になり「一致度◯%」として出せる
- **同点が支配的になる**(選んだジャンルだけを持つゲームは全部100%)。id昇順だけだと
  スラッグ順という無意味な並びになるので、対応機種数→受賞件数→id昇順で崩している
- **データは `generated/recommend-index.json`**(ジャンル表 + 各ゲームのジャンルidだけ)。
  これだけで選択UIが動き、**games.json はジャンルを1つ選んでから**取りに行く。
  **この索引の読み手は /recommend だけ。ページを消すなら生成も消すこと**
- 選択状態はURLの `?tags=<id>,<id>,<id>`。知らないidは黙って捨て、3つ上限は未選択チップの無効化で表現
- **導線はトップの「人気ジャンル」節のみ。TopNav には足していない**(ナビを増やす変更は過去に撤去されている)
- 未選択時に `<Loading />` を出さないこと(`prerender.mjs` が「読み込み中」の消滅を待つため)

### ゲーム起点モード「ゲームから」(2026-08-13、ranobe-dbから移植)

`/recommend` 内のタブ切替で、好きなゲームを最大3本選んで似たゲームを出す第2モード
(`src/ui/recommend/GameRecommendSection.tsx`)。別ルートにはしていない。

- **モード判定は `?games=<id>,<id>,<id>` の有無**(tags= と同居しても games 優先)。`?mode=games` は
  空選択状態の保持用。タブ切替時は相手側のクエリを消す
- **データは games.json だけ**。recommend-index.json は読まないし拡張もしない(主要導線の
  ゲーム詳細「このゲームが好きなら」→「このゲームからもっと探す」では取得済みキャッシュで追加転送ゼロ)
- **スコアは各シードとの類似度(ビルド側 relatedIdsFor と同一式: ジャンルIDFコサイン +
  同開発元+0.15)を個別に計算して算術平均**。ボーナスで1.0を超えるので **%表示は
  `Math.min(100, …)` で頭打ち**
- **同一シリーズのゲームは除外する**(ビルド側 `relatedIdsFor` の `sameSeries()` と同じ)。
  詳細ページにはシリーズ節が別にあるので、ここで続編・リメイクを並べても同じ一覧の繰り返しになる。
  **7サイトのうちシリーズ除外があるのはこのサイトだけ**なので、一括移植では抜けやすい
- **同点がとても多い**(ジャンルの語彙が狭いため、上の例では6件が同スコア)。ビルド側は
  id昇順だけで崩すのに対しこちらは プラットフォーム数→受賞数→id で崩すので、
  **シード1本でも詳細ページの並びとは順序が入れ替わる**(スコア自体は一致する)
- 結果グリッドとタイブレークは `RecommendPage.tsx` の `RecommendGrid` / `tieBreakKey`(export化)を
  両モードで共用。このモードは games.json ロード中に `<Loading />` を出してよい(プリレンダーが
  見る素の `/recommend` はジャンル起点のため)
