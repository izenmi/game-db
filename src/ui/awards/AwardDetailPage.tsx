...
export function AwardDetailPage() {
  ...
  useSeo({
    title: award?.name,
    description: award
      ? `「${award.name}」(主催: ${award.organizer})の受賞作${award.gameCount}件一覧。${award.description}`.slice(0, 160)
      : undefined,
    jsonLd: award
      ? breadcrumbJsonLd([
        ...
        <>
          ...
          <ul className="winner-list">
            {state.data.winners.map((winner) => (
              <li key={`${winner.gameId}-${winner.year}`}>
                <span className={`winner-year winner-year--${colorForYear(winner.year)}`}>{winner.year}</span>
                <Link to={`/games/${winner.gameId}`}>{winner.gameTitle}</Link>
