...
import { getRecommendIndex, getGames } from "../../data/manifest";
import type { RecommendIndex, GameGenerated } from "../../types";
...
/** 選んだジャンルとの一致度。
 *
 *  式は `scripts/generate-manifest.mjs` の関連ゲーム(このゲームが好きなら)と**同じもの**を使う。
 *  同じサイトの中で「似ている」の定義が2つあると、詳細ページの並びとここの並びが食い違うため。
 *
 *    idf(t) = log(N / df(t))            N = 全ゲーム数、df = そのジャンルを持つゲーム数
 *    ‖X‖    = sqrt(Σ_{t∈X} idf(t)²)     ← ゲーム側は「そのゲームが持つ全ジャンル」で取る
 *    一致度  = Σ_{t∈選択∩ゲーム} idf(t)² / (‖選択‖ · ‖ゲーム‖)
 *
 *  ビルド側にある制作者の共通ボーナスはここでは足さない(種になるゲームがないため)。
...
function scoreItems(index: RecommendIndex, selected: string[]) {
  ...
  for (const item of index.items) {
    ...
    // ジャンルが1つも付いていないゲーム(‖ゲーム‖ = 0)はここで落ちる。ビルド側と同じガード。
...
/** 同点をどう並べるか。
 *
 *  一致度は同点が出やすい(選んだジャンルだけを持つゲームはすべて100%になる)。
...
function ThemeRecommendSection() {
  ...
  // ジャンル選択に必要なのは軽量索引だけ。カード描画に要るgames.jsonは
  // ジャンルを1つ選ぶまで取りに行かない — 何も選ばずに離脱する人に大きなJSONを払わせないため。
  ...
  const results = useMemo(() => {
    ...
    return scoreItems(index, selected)
      ...
      .filter((s): s is typeof s & { item: GameGenerated } => s.item !== undefined)
      .sort(
        (a, b) =>
          b.score - a.score ||
    ...
    <>
      ...
      {index && (
        <>
          ...
          {/* 結果のカードも .chip-row を持つので、ジャンル選択行だけを指せるクラスを足しておく。 */}
          ...
          {visibleTags.length === 0 && <EmptyState text="該当するジャンルがありません。" />}

          {/* 未選択のときは Loading を出さないこと。prerender.mjs は「読み込み中」が消えるまで待って
              諦めるので、静的HTMLに「読み込み中…」が焼き付いてクローラーがそれを見ることになる。 */}
          {!hasSelection && <EmptyState text="ジャンルを選ぶとおすすめが表示されます。" />}
