...
export const SITE_NAME = "ゲームDB";
export const SITE_ORIGIN = "https://izenmi.github.io";
export const BASE_PATH = import.meta.env.BASE_URL;
export const SITE_URL = `${SITE_ORIGIN}${BASE_PATH}`;
export const DEFAULT_DESCRIPTION =
  "PS5/Switch/Switch2向けゲームを対応機種・開発元/発売元・受賞歴・ジャンルから検索できるファンデータベース。";
...
export function useSeo({ title, description = DEFAULT_DESCRIPTION, image = DEFAULT_OG_IMAGE, jsonLd }: SeoOptions) {
  useEffect(() => {
    const fullTitle = title ? `${title} | ${SITE_NAME}` : `${SITE_NAME} | PS5/Switch/Switch2ゲーム検索データベース`;
